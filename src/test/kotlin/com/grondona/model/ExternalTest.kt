...
class ExternalTest {
    ...
    inner class ToQuotasUpdatedTests {
        ...
        fun `toQuotasUpdated returns null when the match is already locked`() {
            val dbMatches = (0..9).map { matchFromDB(home = "XX$it", away = "XY$it").copy(code = "$it", startedAt = LocalDateTime.now()) }
        ...
        fun `toQuotasUpdated properly updates the quotas when the match has not started and it is not locked`() {
            val dbMatches =
                (0..9).map { matchFromDB(home = "XX$it", away = "XY$it").copy(code = "$it", startedAt = LocalDateTime.now().plusDays(1L)) }
